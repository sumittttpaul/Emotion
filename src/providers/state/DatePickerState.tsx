...
const DatePickerStateContext = createContext({
  ...
  setDatePickerDialog: {} as Dispatch<
    SetStateAction<Partial<DatePickerStateInterface>>
  >,
});
...
const DatePickerState = ({
  ...
  return (
    <DatePickerStateContext.Provider
      value={{ DatePickerDialog, setDatePickerDialog }}
    >
      {children}
    </DatePickerStateContext.Provider>
  );
...
const useDatePickerState = () => {
  ...
  if (!context) {
    throw new Error(
      'useDatePickerState must be used within a DatePickerStateContext'
    );
  }
  return context;
};
